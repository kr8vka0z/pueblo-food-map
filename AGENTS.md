...
- **The scheduled() orchestration (`src/lib/scheduledTasks.ts`'s `runScheduledTasks`) owns the ping AND both daily-gated jobs riding the same 5-min cron** — extracted out of `custom-worker.ts` itself (which can never carry test coverage: it imports `.open-next/worker.js`, build output vitest can't resolve) so the branching is unit-tested. Each job has its own independent `ctx.waitUntil`, so a slow/failing job never delays or blocks the ping or any other job: **email retention cleanup** (#594, `src/lib/emailRetention.ts`'s `shouldRunEmailRetention`, 09:00 UTC) blanks `public_submissions.submitter_email` (+ the same address inside `payload`) and a `box_adopters` row rejected >90 days ago (row kept); deletes an `alert_subscriptions` row unsubscribed >90 days ago (NOT blanked — `UNIQUE(role, venue_id, email)` + a NOT NULL column means a constant placeholder collides, and `boxAlerts.ts`'s `resubscribeByToken` has no expiry check, so blanking risks resurrecting a subscription with a garbage email); **refresh-pipeline alerts** (#238 pending-age, #234 per-source staleness, `src/lib/refreshAlerts.ts`'s `shouldRunRefreshAlertsCheck`, 09:30 UTC — a distinct slot from retention's so the two never collide) — see "Automated venue-refresh pipeline" below for the alerts themselves. Since staging's `triggers.crons` is `[]`, both jobs' first live run is production; each has its own real-SQLite `.sql.test.ts` as the pre-prod proof.
...
- **Alerts (#238 pending-age, #234 per-source staleness) — `src/lib/refreshAlerts.ts`, riding the existing prod-only 5-min cron via `scheduledTasks.ts`'s `runScheduledTasks`** (see "Observability" above), gated to one daily tick (09:30 UTC — distinct from the email-retention job's own 09:00 UTC slot) so D1 isn't queried 288x/day. Emails `issues@pueblofoodmap.com` via Resend (`env.RESEND_API_KEY`, read from the Workers binding directly — NOT `process.env`, which OpenNext only populates inside a request, not `scheduled()`) at most once a day per condition: (1) any `change_proposals` row has sat `pending` over 14 days, (2) a source (`plentiful`/`osm`) has no `change_proposals` row (of any status) less than 40 days old. No new migration — every successful, non-aborted run writes at least one row per source (an unchanged venue still gets an auto-applied date-only `last_verified` bump every run), so `MAX(created_at)` per source already IS "when that source last completed a run"; a zero-record or abnormal-drop abort correctly writes nothing, which is the silence this alert catches. `ponytail:` ceiling — a source with zero active venues left could complete without writing anything without aborting; not a real case today, upgrade path is a dedicated `refresh_runs` table if it ever is.
- **Hiding a known-dead link (#234):** while a `link_health` finding is still `pending` (an admin hasn't yet resolved it — edit-with-`proposalId` or reject, both flip status away from `pending`), `POST /api/admin/publish` (`fetchPendingDeadLinks` + `stripDeadLinkUrls`, `src/lib/publishVenues.ts`) drops that exact flagged `url` from the snapshot before it's committed to `published-venues.ts` — the public card, `/venue/<id>`, and JSON-LD all read that one snapshot, so this covers all three with no client JS. Compares the venue's CURRENT `url` against the finding's flagged one (not just "this venue id has an open finding"), so a hand-edited replacement url republishes immediately rather than waiting on the stale proposal. **Ceiling:** a pending `link_health` finding alone doesn't light the Publish bar (`summarizePublishChanges` only counts venue-row changes) — hiding only takes effect on whatever the NEXT publish is for any reason, not immediately when the finding is written.
