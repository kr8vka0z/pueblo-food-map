...
- **Email retention cleanup (#594) rides the same 5-min cron**, gated to one of the 288 daily ticks (`src/lib/emailRetention.ts`'s `shouldRunEmailRetention`, 09:00 UTC) so D1 isn't hit 288x/day. Blanks `public_submissions.submitter_email` (+ the same address inside `payload`) and a `box_adopters` row rejected >90 days ago (row kept, per #594); deletes an `alert_subscriptions` row unsubscribed >90 days ago (NOT blanked — `UNIQUE(role, venue_id, email)` + a NOT NULL column means a constant placeholder collides, and boxAlerts.ts's `resubscribeByToken` has no expiry check, so blanking risks resurrecting a subscription with a garbage email). Independent `ctx.waitUntil` from the HC.io ping above — a cleanup failure is caught/logged and never blocks or delays the heartbeat. Since staging's `triggers.crons` is `[]`, this SQL's first live run is production; `src/lib/emailRetention.sql.test.ts` (real SQLite against the actual migrations) is the pre-prod proof.
