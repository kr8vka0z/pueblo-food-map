...
**`0001`–`0012` are all applied to production as of 2026-09-20 (promotion #551)** — `d1_migrations` holds twelve rows, one each. **`0013` is NOT yet applied to production** (outstanding, this promotion) — unlike `0011`/`0012` it IS idempotent (every `UPDATE` is guarded on `id` + exact current `notes` text, so a re-run is a safe no-op), but it still needs the same manual, Kyle-gated `--remote` apply as every migration here. Re-running `migrations apply` is safe because it skips what's tracked; hand-applying `0011` or `0012` with `d1 execute --file` is NOT, since neither is idempotent. **After `0013` lands on production, an admin must click Publish** (`POST /api/admin/publish`) — `src/data/published-venues.ts` is a snapshot regenerated from D1, so the public `/venue/<id>` pages keep showing the old boilerplate notes until the next publish. Pre-migration export from the 2026-09-20 promotion: `~/backups/pfm-prod-d1/prod-20260920-1906-pre-0005-0012.sql` — the only database restore point, as `wrangler rollback` reverts code and cannot undo a migration.
