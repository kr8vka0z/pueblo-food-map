...
Run ALL migrations, in order, against prod D1 (`pueblo-food-map-admin`) BEFORE promoting — `0001_init_admin_schema.sql`, `0002_public_submissions.sql`, `0003_better_auth_schema.sql`, `0004_rate_limit_table.sql`, `0005_blessing_boxes.sql`, `0006_convert_routt_blessing_box.sql`, `0007_box_checkins.sql`, `0008_box_events.sql`, `0009_box_photos.sql`, `0010_box_adopters_alerts.sql`, `0011_alert_email_lang.sql`, `0012_box_checkin_needs.sql` (must follow `0010`/`0011` respectively, each run exactly once — see "Blessing Boxes" above). `npx wrangler d1 migrations apply pueblo-food-map-admin --remote`; `deploy-dev.yml` applies staging's copy automatically on every push to `dev`, `deploy-prod.yml` does NOT — production migration stays a manual, Kyle-gated step.

**`0001`–`0012` are all applied to production as of 2026-09-20 (promotion #551)** — `d1_migrations` holds twelve rows, one each. Only a new migration needs running. Re-running `migrations apply` is safe because it skips what's tracked; hand-applying `0011` or `0012` with `d1 execute --file` is NOT, since neither is idempotent. Pre-migration export from that promotion: `~/backups/pfm-prod-d1/prod-20260920-1906-pre-0005-0012.sql` — the only database restore point, as `wrangler rollback` reverts code and cannot undo a migration.
