...
**Current state:** admin is gated by Better Auth ALONE — magic link + passkey, one-email allowlist. Cloudflare Access has been removed from the admin path entirely (no Access app, no JWT header, no JWKS check) on `dev`/staging; **production cutover is a separate, later, explicit step** (also requires deleting the live CF Access application from Cloudflare's dashboard — outside this repo). `requireAccessIdentity()` no longer exists in the code at all.
...
- **`requireAdminOrigin()`** (`src/lib/cfAccess.ts`) is CSRF, independent of identity — required on every non-GET `/api/admin/*` mutation.
...
- **`/admin`** lists every `venues` row; **`POST`/`PATCH /api/admin/venues[/id]`** create/edit (full server-side re-validation, `src/lib/adminVenueValidation.ts` — SQLite CHECK constraints don't cover lat/lng bounds or the hours JSON shape); **`POST .../archive`** sets `status='archived'` (never `DELETE`). Every mutation is one atomic `db.batch()` + an `audit_log` row. Address→coordinates uses the free US Census geocoder (`geocoding.geo.census.gov`), not Mapbox — no key/allowlist to manage.
- **`POST /api/admin/publish`** (`src/lib/publishVenues.ts`) snapshots `draft`+`published` rows, commits `src/data/published-venues.ts` to a fixed `publish-bot` branch via the GitHub API, opens/reuses its auto-merging PR — **only once that GitHub sequence succeeds** does it promote drafts in D1 (`db.batch()`). **This ordering is load-bearing** (regression-tested): get it backwards and D1 can mark rows "published" when the file never shipped. `GITHUB_PUBLISH_TOKEN` — fine-grained PAT scoped to this repo only, Contents RW + Pull requests RW — provisioned (#260); confirm it's still set on whichever environment you're promoting to before relying on Publish there.
...
- **Migrations, in order:** `0005_blessing_boxes.sql` (schema + the `venues` CHECK-constraint widen), `0006_convert_routt_blessing_box.sql`, `0007_box_checkins.sql` (+ `box_checkin_rate_limit`), `0008_box_events.sql`, `0009_box_photos.sql`, `0010_box_adopters_alerts.sql`, `0011_alert_email_lang.sql` — **0011 must run AFTER 0010 and is NOT idempotent** (`ALTER TABLE ADD COLUMN` has no `IF NOT EXISTS`; a second run fails "duplicate column name"). Apply with `wrangler d1 migrations apply` (tracked), never `d1 execute --file`, for anything not idempotent.
...
Run ALL migrations, in order, against prod D1 (`pueblo-food-map-admin`) BEFORE promoting — `0001_init_admin_schema.sql`, `0002_public_submissions.sql`, `0003_better_auth_schema.sql`, `0004_rate_limit_table.sql`, `0005_blessing_boxes.sql`, `0006_convert_routt_blessing_box.sql`, `0007_box_checkins.sql`, `0008_box_events.sql`, `0009_box_photos.sql`, `0010_box_adopters_alerts.sql`, `0011_alert_email_lang.sql` (must follow `0010`, run exactly once — see "Blessing Boxes" above). `npx wrangler d1 migrations apply pueblo-food-map-admin --remote`; `deploy-dev.yml` applies staging's copy automatically on every push to `dev`, `deploy-prod.yml` does NOT — production migration stays a manual, Kyle-gated step.
