...
- BottomNav (docs/bottom-nav-spec.md): Near me · Saved · Resources · Menu (Resources links to the `/resources` page; Saved and Menu open the drawer). Below `2xl` (1536px) a 78px `bone-50` bar across the bottom (icon above a 12px/700 label, `ink-500` / `brand-navy` when its panel is open), with a non-interactive fade band above it; at `2xl`+ a white pill beside the search box
