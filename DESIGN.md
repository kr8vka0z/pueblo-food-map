...
**Venue phone link** (`BottomSheet`, `DesktopVenueWindow`, `VenueContent`): the `tel:` number is a plain underlined text link — `sage-700`, `font-semibold`, `underline underline-offset-2`, `sage-600` phone icon, `min-h-11` on touch surfaces. Deliberately a link, not a button (Kyle, 2026-09-23, option A of four mockups): calling is a convenience here, not the card's main action.
...
- `SiteFooter`'s four internal links are plain `text-xs` lines, under the 44px tap-target floor (only its OSM link is `min-h-11`). `inline-flex items-center min-h-11` on each would fix it without changing the look.
