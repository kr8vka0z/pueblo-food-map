...
- **App: `REFRESH_INGEST_TOKEN`** — a bearer secret, set via
  ...
  (`op://Atlas/...`) and provided to the GitHub Action as a repo secret (same
...
- **Edge: a Cloudflare Access Service Token** (§3.1, I4) — lets the GitHub
  ...
  `CF-Access-Client-Secret`, 1Password `op://Atlas/...`, GitHub Actions repo
...
- **App: `REFRESH_INGEST_TOKEN`** — bearer secret, `wrangler secret put`,
  1Password `op://Atlas/...`, GitHub Actions repo secret. Checked inside the
...
- **Secrets** (all in Kyle's existing `op://Atlas/...` 1Password vault; most
...
| Secrets | Kyle's existing 1Password VPS vault (`op://Atlas/...`), same pattern as every other PFM credential (Mapbox, Resend, Turnstile) | A new Firebase service-account JSON key — a new *kind* of credential this project has never held before |
