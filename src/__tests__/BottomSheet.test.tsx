...
// ─── #531: onStripVisibleChange (MapWrapper's only way to learn whether the ───
// strip or the full card is showing, since `cardRevealed` is BottomSheet's
// own internal state) — the mocked vaul above ignores snapPoints/
// activeSnapPoint entirely and only reads `open`, so `showStrip` (computed
// from isWalkRouteActive + cardRevealed) still renders RouteStrip correctly
// without any real vaul drag machinery involved.
describe("BottomSheet — onStripVisibleChange (#531)", () => {
  test("full card (no route): reports false and renders the ordinary card, not the strip", () => {
    const onStripVisibleChange = vi.fn();
    render(<BottomSheet venue={makeVenue()} onClose={() => {}} onStripVisibleChange={onStripVisibleChange} />);
    expect(onStripVisibleChange).toHaveBeenCalledWith(false);
    expect(screen.queryByTestId("route-strip")).toBeNull();
  });

  test("route active on mount: reports true and renders the strip instead of the full card", () => {
    const onStripVisibleChange = vi.fn();
    render(
      <BottomSheet
        venue={makeVenue()}
        onClose={() => {}}
        isWalkRouteActive={true}
        onStripVisibleChange={onStripVisibleChange}
      />,
    );
    expect(onStripVisibleChange).toHaveBeenCalledWith(true);
    expect(screen.getByTestId("route-strip")).toBeDefined();
    // The full card's own toggle must NOT be present while the strip shows.
    expect(screen.queryByRole("button", { name: /show details/i })).toBeNull();
  });

  test("tapping 'Show card' on the strip flips the report back to false", async () => {
    const onStripVisibleChange = vi.fn();
    const user = userEvent.setup();
    render(
      <BottomSheet
    ...
    onStripVisibleChange.mockClear();
  ...
  test("route clearing (isWalkRouteActive flips back to false) reports false", () => {
    const onStripVisibleChange = vi.fn();
