/**
 * FeedbackPageContent bilingual rendering tests (pueblo-food-map#bilingual-static-pages).
 *
 * Proves the extracted client component (heading + FeedbackForm, both now
 ...
 * dedicated FeedbackForm test suite does.
 */
...
describe("FeedbackPageContent — locale", () => {
  test("renders English heading and form labels with no provider (default locale)", () => {
    render(<FeedbackPageContent />);
    expect(
      screen.getByRole("heading", { level: 1, name: t("feedback.title", "en") }),
    ).toBeDefined();
    expect(screen.getByLabelText(new RegExp(t("feedback.type.label", "en")))).toBeDefined();
  });
  ...
  test("renders Spanish heading and form labels when locale='es'", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: t("feedback.title", "es") }),
    ).toBeDefined();
    expect(screen.getByLabelText(new RegExp(t("feedback.type.label", "es")))).toBeDefined();
  });
});
