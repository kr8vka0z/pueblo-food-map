 ...
 * of how many statements run in between. Not yet exercised against a real
 * D1/SQLite instance in this repo (only Cloudflare's public batch()/
 * RETURNING docs and the mocked route tests) — flagged in the PR as the one
...
const BOX_DELETE_SQL =
  "DELETE FROM blessing_boxes WHERE venue_id = ? AND EXISTS (SELECT 1 FROM venues WHERE id = ? AND updated_at = ?)";
const BOX_INSERT_SQL = `INSERT INTO blessing_boxes (venue_id, host_name, host_note, host_contact, most_needed, installed_on, removed_on)
  SELECT ?, ?, ?, ?, ?, ?, ?
  WHERE EXISTS (SELECT 1 FROM venues WHERE id = ? AND updated_at = ?)`;
...
const BOX_EVENT_INSERT_SQL_GUARDED = `INSERT INTO box_events (venue_id, kind, detail, created_at)
  SELECT ?, ?, ?, ?
  WHERE EXISTS (SELECT 1 FROM venues WHERE id = ? AND updated_at = ?)`;
...
// statements above.
const APPROVE_PROPOSAL_SQL = `UPDATE change_proposals SET status = 'approved', reviewed_by = ?, reviewed_at = ?, applied_at = ?
